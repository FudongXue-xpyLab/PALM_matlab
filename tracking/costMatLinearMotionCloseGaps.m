function [costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,...
    ...
    kalmanFilterInfo,nnDistLinkedFeat,probDim,handles)
...
for iFrame = 1 : numFrames - 1
    if floor(iFrame/(numFrames-1)*50)>floor((iFrame-1)/(numFrames-1)*50)
    mywaitbar(iFrame/(numFrames-1)/2,handles.axes2,[num2str(floor(iFrame/(numFrames-1)*50)),'%']);
    end
...
for iPair = 1 : numPairs
    if floor(iPair/numPairs*50)>floor((iPair-1)/numPairs*50)
    mywaitbar(iPair/numPairs/2+0.5,handles.axes2,[num2str(floor(iPair/numPairs*50+50)),'%']);
    end
    ...
    %modified by ZYD
    cen2cenVecMag = max(cen2cenVecMag,0.1);
