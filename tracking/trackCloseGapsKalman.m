...
%         disp('Linking features forwards ...');
    set(handles.text10,'string','Linking features forwards ...')
    end
    [dummy,dummy,kalmanInfoLink,dummy,linkingCosts] = linkFeaturesKalman(...
        ...
        kalmanFunctions,probDim,[],[],verbose,handles);
...
%         disp('Linking features backwards ...');
    set(handles.text10,'string','Linking features backwards ...')
    end
    [dummy,dummy,kalmanInfoLink,dummy,linkingCosts] = linkFeaturesKalman(...
        movieInfo(end:-1:1),costMatrices(1).funcName,costMatrices(1).parameters,...
        kalmanFunctions,probDim,kalmanInfoLink(end:-1:1),linkingCosts,verbose,handles);
    clear dummy dummy1
...
%         disp('Linking features forwards ...');
    ...
    [tracksFeatIndxLink,tracksCoordAmpLink,kalmanInfoLink,nnDistLinkedFeat,...
        ...
        kalmanInfoLink(end:-1:1),linkingCosts,verbose,handles);
...
%         disp('Linking features ...');
    set(handles.text10,'string','Linking features ...')
    end
    [tracksFeatIndxLink,tracksCoordAmpLink,dummy,nnDistLinkedFeat,...
        dummy,errFlag] = linkFeaturesKalman(movieInfo,costMatrices(1).funcName,...
        costMatrices(1).parameters,kalmanFunctions,probDim,[],[],verbose,handles);
    kalmanInfoLink = [];
...
%         progressText(0,'Gap closing');
        set(handles.text10,'string','Gap closing ...')
...
%         'gapCloseParam,kalmanInfoLink,nnDistLinkedFeat,probDim);'])
    [costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,errFlag] = costMatLinearMotionCloseGaps(tracksCoordAmpLink,tracksFeatIndxLink,trackStartTime,trackEndTime,costMatrices(2).parameters,gapCloseParam,kalmanInfoLink,nnDistLinkedFeat,probDim,handles);
    ...
    if any(isfinite(nonzeros(costMat)))
        ...
        for iTrack = 1 : numTracksCG
            ...
            if floor(iTrack/numTracksCG*100)>floor((iTrack-1)/numTracksCG*100)
            mywaitbar(iTrack/numTracksCG,handles.axes2,[num2str(floor(iTrack/numTracksCG*100)),'%']);
            end
...
%         set(handles.text10,'string','Gap closing ...');
        mywaitbar(1,handles.axes2,[num2str(floor(100)),'%']);
    end
    ...
    if verbose
%         disp('No gaps to close!');
    set(handles.text10,'string','No gaps to close!');
    mywaitbar(1,handles.axes2,[num2str(floor(100)),'%']);
    end
