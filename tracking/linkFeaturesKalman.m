...
%                 'probDim,prevCost,featLifetime);'])
             [costMat,propagationScheme,kalmanFilterInfoTmp,nonlinkMarker] = costMatLinearMotionLink(movieInfo(iFrame:iFrame+1),kalmanFilterInfo(iFrame),costMatParam,nnDistFeatures(1:numFeaturesFrame1,:),probDim,prevCost,featLifetime);
...
%                             'filterInfoPrev(iFrame+1));'])
                    [kalmanFilterInfo,errFlag] = kalmanGainLinearMotion(trackedFeatureIndx(1:numFeaturesFrame2,:),movieInfo(iFrame+1),kalmanFilterInfoTmp,propagationScheme,kalmanFilterInfo,probDim,filterInfoPrev(iFrame+1));    
                    else %if no prior information is supplied
%                         eval(['[kalmanFilterInfo,errFlag] = ' kalmanFunctions.calcGain ...
%                             '(trackedFeatureIndx(1:numFeaturesFrame2,:),'...
%                             'movieInfo(iFrame+1),kalmanFilterInfoTmp,'...
%                             'propagationScheme,kalmanFilterInfo,probDim,[]);'])
                    [kalmanFilterInfo,errFlag] = kalmanGainLinearMotion(trackedFeatureIndx(1:numFeaturesFrame2,:),movieInfo(iFrame+1),kalmanFilterInfoTmp,propagationScheme,kalmanFilterInfo,probDim,[]);    
                    end
...
%                             '(movieInfo(iFrame+1),probDim);'])
                        [filterInit,errFlag] = kalmanInitLinearMotion(movieInfo(iFrame+1),probDim);
...
%         axes(handles.axes2)
        if iFrame/(numFrames-1)*100>(iFrame-1)/(numFrames-1)*100
        mywaitbar(iFrame/(numFrames-1),handles.axes2,[num2str(floor(iFrame/(numFrames-1)*100)),'%']);
        end
    end
