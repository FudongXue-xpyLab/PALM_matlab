...
meanSigmaflag=VH.parameter.reconstruction.meanSigma;
...
mywaitbar(0,VH.axes2,'');
for i=1:m
     if  VH.fitInfo(i,7)>minPhoton && VH.fitInfo(i,7)<maxPhoton && VH.fitInfo(i,6)<maxError && VH.fitInfo(i,8)<maxBg && VH.fitInfo(i,4)>minSigma && VH.fitInfo(i,4)<maxSigma
...
%              Reconstruct_I6(ccx,ccy)=Reconstruct_I6(ccx,ccy)+1;
             if VH.parameter.fitting.flag==4  
                 if VH.fitInfo(i,9)>=1 && VH.fitInfo(i,9)<=300
%                      z=round((VH.fitInfo(i,9))/2);
                     z1=VH.fitInfo(i,9)-160;
                     if z1>0
                         z=z1/2;
                     else
                         z=(z1+300)/2;
...
FileStr=strcat(filename,'_reconstruct','.tif');
imwrite(Reconstruct_I1,FileStr,'tif','Compression','none','WriteMode','overwrite')
FileStr=strcat(filename,'_reconstruct_dot','.tif');
...
% end
fstr=strcat(VH.pathname,VH.filebase,'_position.txt');
dlmwrite(fstr, V, 'delimiter', '\t','precision', 6);
