...
ROI = [];
% for i=1:m
%     fitInfo1(i,:)=fitInfo1(i,:)/max(fitInfo1(i,:));
% end
% P=importdata('Calibration.mat');
...
for i=1:m
    ...
    left=round(cy)-fitl;
    right=round(cy)+fitl;       
    if  up>=1 && left>=1 && bottom<=row  && right<=column  && n<VH.ImageNumber % && n>5  %&& abs(fitInfo(i,1)-fitl-1)<1 && abs(fitInfo(i,2)-fitl-1)<1 && fitInfo(i,1)~=round(fitInfo(i,1)) && fitInfo(i,2)~=round(fitInfo(i,2))
        I=ROIfit(:,:,i);
        VH.ROI(:,:,mm)=I;
