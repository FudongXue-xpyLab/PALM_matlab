...
for i=1:N+1
    ...
    tic
    [X Y Np BG S CRLBx CRLBy CRLBn CRLBb CRLBs LogL]=mGPUgaussMLE(single(ROI),sigma,20,2);
    toc
    fitInfo(st:et,1)=X+1;
    fitInfo(st:et,2)=Y+1;
    fitInfo(st:et,3)=Np;
...
for i=1:m
    ...
    if up>=1 && left>=1 && bottom<=row  && right<=column  && n<VH.ImageNumber % && abs(fitInfo(i,1)-fitl-1)<1 && abs(fitInfo(i,2)-fitl-1)<1 && fitInfo(i,1)~=round(fitInfo(i,1)) && fitInfo(i,2)~=round(fitInfo(i,2))
        ...
        if abs(fitInfo(i,1)-fitl-1)<2 && abs(fitInfo(i,2)-fitl-1)<2 && abs(fitInfo(i,1)-fitl-1)>0 && abs(fitInfo(i,2)-fitl-1)>0 ...
            && abs(fitInfo(i,1)-fitl-1-(cx-round(cx)))<1 && abs(fitInfo(i,2)-fitl-1-(cy-round(cy)))<1
